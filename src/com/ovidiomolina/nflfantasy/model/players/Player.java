...
public abstract class Player {
    ...
    protected Player(Database db,String position, String playerId, String playerName, String playerTeam) {
        this.db = db;
    ...
    public static Player createPlayer(Database db, String position, String playerId, String playerName, String playerTeam) {
    ...
    public void train() {
        Thread fantasyPtsTrainThread = new Thread() {
                public void run() {
                    ...
                    getLearningRate(), getHiddenNodes());
    ...
    public List<Double> getXLabelValues(DefaultTableModel model) {
        ...
        for(int i = 0; i<model.getRowCount(); i++) {
            xLabelValues.add((double)model.getValueAt(i, PlayerConstants.WEEK_COL));
        }
        return xLabelValues;
    }
    ...
    public List<ChartItem> getChartItems(DefaultTableModel model, int[] columns) {
        ...
        for(int row = 0; row<model.getRowCount(); row++) {
            for(int j = 0; j<columns.length; j++) {
                plots.get(j).add((double) model.getValueAt(row, columns[j]));
            }
        }
        for(int i = 0; i<columns.length; i++) {
